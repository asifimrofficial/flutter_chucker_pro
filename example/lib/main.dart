...
Future<void> main() async {
  ...
  runApp(const ChuckerExampleApp());
}

class ChuckerExampleApp extends StatelessWidget {
  const ChuckerExampleApp({super.key});
...
class _ExampleHomePageState extends State<ExampleHomePage> {
  late final Dio _dio = Dio(
    BaseOptions(
      ...
      headers: <String, Object?>{'Authorization': 'Bearer example-token'},
    ),
  )..interceptors.add(ChuckerInterceptor());

  bool _loading = false;
  String _lastResult = 'Ready';
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('Flutter Chucker Pro'),
        actions: <Widget>[
          IconButton(
            tooltip: 'Open logs',
  ...
  Future<void> _createPost() async {
    await _run(() async {
      final response = await _dio.post<Map<String, dynamic>>(
        ...
        data: <String, Object?>{
          'title': 'Flutter Chucker Pro',
          'body': 'Captured from the example app',
          'userId': 1,
        },
      );
